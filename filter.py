            ...
            for customer in customers:
                if customer.get_id == filter_string:
                    for num in customer.get_phone_numbers():
                        customer_nums.append(num.number)

            if len(customer_nums) == 0:
                return data

            else:
                for call in data:
                    if call.src_number in customer_nums:
                        current_data.append(call)
                    elif call.dst_number in customer_nums:
                        current_data.append(call)
        ...
        if len(filter_string) > 0:
            if filter_string[0] in valid:
                try:
                    ...
                    op = filter_string[0]
                    filter_int = int(filter_string[1:])
                    final = []
                    if op == 'G':
                        for call in data:
                            if call.duration > filter_int:
                                final.append(call)
                    elif op == 'L':
                        for call in data:
                            if call.duration < filter_int:
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        try:
            ...
            valid = [-79.196382, 43.799568, -79.697878, 43.576959]
            if len(contain) != 4:
                return data

            elif range[0] > valid[0] or range[0] < valid[2]:
                return data

            elif range[1] > valid[1] or range[1] < valid[3]:
                return data
                ...
                for call in data:
                    call_data = [[],[]]
                    call_data[0].append(call.src_loc[0])
                    call_data[0].append(call.src_loc[1])
                    call_data[1].append(call.dst_loc[0])
                    call_data[1].append(call.dst_loc[1])

                    if call_data[0][0] >= contain[0] and call_data[0][0] <= contain[2]:
                        if call_data[0][1] >= contain[1] and call_data[0][1] <= contain[3]:
                            final.append(call)

                    elif call_data[1][0] >= contain[0] and call_data[1][0] <= contain[2]:
                        if call_data[1][1] >= contain[1] and call_data[1][1] <= contain[3]:
                            final.append(call)
                return final
